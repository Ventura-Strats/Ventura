...
class ibApp(EWrapper, EClient):
    def __init__(self, account_id):
        ...
        self.asset_list = buildAssetList(self.account_id)
...
def buildAssetList(account_id):
    dat = getDBAssetData()
    if account_id == 1:
        dat = dat[~dat.asset_class.isin(["fx_em", "fx_dm"])]
    elif account_id == 2:
        dat = dat[dat.asset_class.isin(["fx_em", "fx_dm"])]
...
if __name__ == "__main__":
    ...
    for account_id in [1,2]:
        ib_app = ibApp(account_id)
        ib_app.connect("127.0.0.1", xu.getIBPort(account_id), 98)#xu.getIBClientId(script_name))      
        ib_app.run()
        ib_app.disconnect()
